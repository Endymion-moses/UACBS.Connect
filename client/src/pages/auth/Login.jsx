...
const Login = () => {
    ...
    const [error, setError] = useState(""); // Captures API failure logs
            ...
            const response = await fetch(`${import.meta.env.VITE_API_BASE_URL}/auth/login`, {
                ...
                body: JSON.stringify({
                    ...
                    role: selectedRole
                            ...
                            // 1. First check if the response status code is healthy (200-299)
                if (!response.ok) {
                    // If the server sent HTML error text, read it safely without crashing
                    const errorText = await response.text();

                    // Check if it's an HTML page or a structured message
                    if (errorText.includes("<!DOCTYPE")) {
                        throw new Error(`Server Route Error (${response.status}): The URL path requested does not exist on the server.`);
                    } else {
                ...
                // 2. Safely decode now that we confirmed it's valid JSON
                ...
                const actualRole = data.user.role.toUpperCase();

                if (actualRole !== selectedRole) {
                    throw new Error(`This account is registered as ${actualRole.toLowerCase()}. Please login using the ${actualRole.toLowerCase()} tab or use the correct account.`);
                }

                // 4. Route dynamically based strictly on verified database permissions
                if (actualRole === "STUDENT") {
                    ...
                    setError("Unauthorized role type detected.");
                }
    ...
    return (
        <div className="flex md:flex-row min-h-screen">
            <div className = "hidden md:flex w-full md:w-1/2 bg-blue-900 flex-col mx-auto px-10">
                ...
                <div className="pt-10" >
                    <p className="text-white text-5xl p-4">Book Consultations with your lecturers - online, anytime</p>
                    <p className="text-gray-400 text-lg p-5">no more wondering office to office. Find available slots, submit your request, and get notified instantly </p>
                </div>
            ...
            <div className = "w-full md:w-1/2 flex flex-col items-center justify-center bg-gray-100">
                <div>
                    ...
                    {/* Error Box Notification Wrapper */}
                    {error && (
                        <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-2 rounded-xl mb-4 text-sm max-w-sm">
                            {error}
                        </div>
                    )}
                    ...
                    <form onSubmit={handleSubmit}>
                        ...
                        <div className= "flex justify-between items-center my-4">
                            ...
                            <Link className="text-blue-500 hover:text-blue-800">Forgot Password?</Link>
                        </div>

                        <button type="submit" disabled={isLoading} className="bg-blue-900 w-full rounded-xl p-3 text-white font-bold disabled:opacity-50">
